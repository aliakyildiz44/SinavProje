...
namespace SinavProjesi
{
    public partial class LoginFormm : Form
    {
        ...
        {
            int count = 0;
            using (SqlConnection sql = new SqlConnection(@"Data Source = ALI; Initial Catalog = ExamApplication; Integrated Security = True"))
            {
                sql.Open();
        ...
        private void LoginButton_Click(object sender, EventArgs e)
        {
            ...
            bool control = false;
            durum = false;
            using (SqlConnection con = new SqlConnection(@"Data Source=ALI;Initial Catalog=ExamApplication;Integrated Security=True"))
            {
                ds = new DataSet();
                cmd = new SqlCommand();
                con.Open();
                ...
                if (control == false)
                {
                    cmd = con.CreateCommand();
                      cmd.CommandText = "Select * from Users where Email='" + useremail + "'AND Password='" + userpassword + "'";
                   // cmd.CommandText = "SELECT * FROM Users u inner join Students s on u.UserId = s.UserId where u.Email ='" + useremail + "' and u.Password = '" + userpassword + "'";
                    ...
                    if (UserTypeId == 1)
                    {
                        ...
                        cmd2.CommandText = "Select * from Students where UserId='" + Id + "'";
                        SqlDataAdapter da2 = new SqlDataAdapter(cmd2);
                        da2.Fill(ds2);
                        object[] dt2 = ds2.Tables[0].Rows[0].ItemArray;
                        StudentId = Convert.ToInt32(dt2[0]);
                        User student = new User(Id, Username, Name, Surname, Email, Password);
                        StudentsScreen studentScreen = new StudentsScreen(student,StudentId);
                        studentScreen.Show();
                    ...
                    else
                    {
                        ...
                        EducatorForm educatorScreen = new EducatorForm();
                        educatorScreen.Show();
                        MessageBox.Show("Welcome");
                        this.Hide();
                    }
